...
class SpeechToText:
    ...
    def speech_to_text_from_bytes(self, audio_bytes: bytes):
        """
        Transcribe an audio file to text from bytes input.

        Parameters:
            audio_bytes (bytes): The content of the audio file as bytes.
        ...
        # Verify ffmpeg and ffprobe binaries
        if not (os.path.isfile(ffmpeg_path) and os.access(ffmpeg_path, os.X_OK)):
            raise FileNotFoundError(f"FFmpeg binary not found or not executable at {ffmpeg_path}.")

        if not (os.path.isfile(ffprobe_path) and os.access(ffprobe_path, os.X_OK)):
            raise FileNotFoundError(f"FFprobe binary not found or not executable at {ffprobe_path}.")

        # Directly use the passed bytes
        audio_file_bytes = io.BytesIO(audio_bytes)
        audio_file_bytes.name = "audio_input"  # Give a dummy name
        audio_file_bytes.seek(0)

        audio = AudioSegment.from_file(audio_file_bytes)

        # The rest remains the same (splitting if longer than 60 sec, transcribing)
        if len(audio) <= 60000:
            buffer = io.BytesIO()
            audio.export(buffer, format="wav", codec="pcm_s16le")
            buffer.name = "audio.wav"
            ...
            chunk_duration_ms = 120 * 1000  # 2 minutes
            chunks = []
            for i in range(0, len(audio), chunk_duration_ms):
                chunk = audio[i:i + chunk_duration_ms]
                buffer = io.BytesIO()
            ...
            def transcribe(chunk_io):
                chunk_io.seek(0)
                print(f"Transcribing chunk of size: {len(chunk_io.getvalue())} bytes")
                transcription = self.client.audio.transcriptions.create(
                    model="gpt-4o-transcribe",
                    file=chunk_io,
                    response_format="json"
